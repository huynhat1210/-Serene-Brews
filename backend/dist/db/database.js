...
async function initializeDatabase() {
    ...
    `);
        ...
        try {
            await exports.pool.query('ALTER TABLE orders ADD COLUMN delivery_method VARCHAR(50) DEFAULT "pickup"');
        }
        catch (e) { }
        try {
            await exports.pool.query('ALTER TABLE orders ADD COLUMN delivery_address VARCHAR(500) DEFAULT NULL');
        }
        catch (e) { }
    ...
    `);
        // Seed Vouchers Table
        const [vRows] = await exports.pool.query('SELECT COUNT(*) as count FROM vouchers');
        ...
        if (count > 0) {
            const [images] = await exports.pool.query('SELECT image FROM products LIMIT 50');
        ...
        if (count !== 25 || hasInvalidImages) {
            ...
            await exports.pool.query('DROP TABLE IF EXISTS group_order_items');
            await exports.pool.query('DROP TABLE IF EXISTS order_items');
            await exports.pool.query('DROP TABLE IF EXISTS products');
