...
app.get('/api/vouchers', async (req, res) => {
    try {
        const [rows] = await database_1.default.query('SELECT * FROM vouchers');
...
app.post('/api/group-orders', async (req, res) => {
    ...
    try {
        await database_1.default.query('INSERT INTO group_orders (id, creator_name) VALUES (?, ?)', [roomId, creatorName || 'Khách hàng']);
        res.json({ roomId });
    }
    catch (err) {
...
app.get('/api/group-orders/:roomId', async (req, res) => {
    ...
    try {
        const [rooms] = await database_1.default.query('SELECT * FROM group_orders WHERE id = ?', [roomId]);
        if (rooms.length === 0) {
            return res.status(404).json({ error: 'Không tìm thấy nhóm đặt đơn này.' });
        }
        const room = rooms[0];
        const [items] = await database_1.default.query('SELECT * FROM group_order_items WHERE group_order_id = ?', [roomId]);
        res.json({
            roomId: room.id,
...
app.delete('/api/group-orders/:roomId', async (req, res) => {
    const { roomId } = req.params;
    try {
        await database_1.default.query('DELETE FROM group_orders WHERE id = ?', [roomId]);
